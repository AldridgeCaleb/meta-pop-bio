...
The `spmm.readxl` function is a convenience function for reading in metadata and metapopulation matrices from an `.xlsx` file. A template ([`spmm-workbook-template.xlsx`](man/spmm-workbook-template.xlsx)) and an example workbook ([`spmm-example-workbook.xlsx`](man/spmm-example-workbook.xlsx)) are provided. The example workbook is used to illustrate the `spmm.readxl` functionality below.
